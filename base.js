...
class TokenList {
    constructor(e) {
        const t = e => /{(\w+|#)}/.test(e.textContent)
          && (e.text = e.textContent).match(/[^{]+(?=})/g)
            .map(t => (
              this[t] || (this[t] = [])).push(e)
            ),
...
(e => {
    ...
    function s(e, t, n) {
        ...
        ["id", "rel", "href", "src", "textContent", "as", "defer", "crossOrigin"].map(e => t[e] && e in o && (o[e] = t[e])), "style" == s
          && (o.rel = "stylesheet"), "script" == s && (o.src = o.href), e.parentNode.insertBefore(o, n)
    }
    ...
    new MutationObserver(e => {
        for (let n of e)
            for (let e of n.addedNodes) /^p/.test(e.rel) && /\-/.test(e.id)
              && t(e), /\-/.test(e.localName)
              && (link = document.querySelector("#" + e.localName))
              && link.content
              && o.call(e, link.content)
...
const Custom = e => (
  class extends(ParentNode(EventTarget(GlobalEventHandlers(e)))) {
    connectedCallback() {
        this.context = {}, super.initialize && super.initialize(), Object.getOwnPropertyNames(e.prototype).map(this.reflect, this),
        this.addEventListener("connect", this.onconnect),
        this.addEventListener("idle", super.onidle), this.dispatchEvent(new Event("connect")),
        this.render()
    }
    render() {
